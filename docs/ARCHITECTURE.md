...
The first mobile surface is a UI lab, not production vault writing. `apps/mobile/src/components/ui` owns RNR-derived shadcn-style primitives such as text, buttons, and badges, while `apps/mobile/src/ui` owns Tolaria-specific wrappers such as icon buttons, panels, toolbars, list rows, chips, and property rows. Read-only workspace components live under `apps/mobile/src/components/workspace` and consume `MobileWorkspaceSnapshot` from `apps/mobile/src/workspace/mobileWorkspaceModel.ts`. The current repository is fixture-backed for screenshot speed, but the tablet shell already consumes the read-only snapshot boundary that future native vault loading should provide. This keeps UI quality work deterministic: each mobile surface can be rendered, clicked, screenshot-tested, and compared against the desktop visual language before storage/editor/Git mutation behavior is attached.
