...
`pnpm mobile:qa:ios-workspace-persistence` and `pnpm mobile:qa:ios-phone-workspace-persistence` exercise reducer write plans through the native Expo filesystem repository. The proof must cover note create/save/move/delete/restore, typed property writes, relationship add/remove and target creation, folder writes including restore, saved-view create/update/delete/reorder/restore writes, Type definition create/update/delete/rename/reorder/restore changes, wikilink rewrites, note metadata, vault-scoped config, and property display-mode hydration.
