...
| General keyboard shortcuts should work (`Cmd+O`, `Cmd+P`, `Cmd+F`, `Cmd+K`, `Cmd+\`) | Partial | Parser/model coverage exists in `mobileWorkspaceKeyboardShortcuts.test.ts`; native listener installation now ignores partial Expo Go `window`/`document` shims instead of crashing, but native Expo Go hardware-key dispatch is still not proven. June 24 simulator probe: `Cmd+K` did not open the command palette. This likely needs a native/dev-client key-command bridge or another focused strategy. |
