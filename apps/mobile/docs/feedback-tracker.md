...
| Modals/action sheets such as section edit, view edit, command palette should be properly designed | Partial | Shared action-sheet spacing now has an explicit native layout contract; suggestion action sheets use the same padded scroll container as the other sheets; command palette input/rows/footer are tied to desktop palette density; workspace form sheets no longer autofocus and trigger the native keyboard over the sheet on first open; long form sheets use a numeric native height derived from the simulator window so content scrolls instead of clipping behind workspace chrome. Type-section schema editors, view filter builders, and display-property pickers now share explicit grouped form-section spacing/padding/radius contracts. Expo QA URLs can open sheets directly with `actionSheet=editTypeSection`, `actionSheet=createView`, `actionSheet=addProperty`, and related targets. Native iPhone proof on the real Laputa dev vault shows create-view opens with compact sort controls, reachable filter controls, capped suggestions, and display-property option buttons (`/tmp/tolaria-create-view-final-top.jpg`, `/tmp/tolaria-create-view-final-filter.jpg`, `/tmp/tolaria-create-view-final-display-properties.jpg`). Remaining modal work: command palette and type-section final native screenshots, plus tablet-specific sheet layering proof. |
...
| All transitions should be very smooth like the reference video | Partial | Tablet transitions are animated and tested at math level; phone committed swipes now settle through the native drag animation before state handoff, and the committed target state is marked atomically so the replacement screen does not inherit the completed drag offset. Native iPhone proof on real Laputa content (`/tmp/tolaria-phone-transition-fixed-4.mp4`) shows editor -> note-list settles into one clean note-list surface. Needs tablet frame/interaction proof before this can be considered fully proved. |
