...
- Tablet action sheets are clickable and tested against the editable snapshot. Create/property/relationship/body edits, Type template editing and template-backed note creation, relationship target creation, saved-view edits, note type changes, note file moves, and folder moves emit repository write plans; explicit `source=native-vault` mode persists those writes through Expo FileSystem, while fixture and host-vault QA remain deterministic.
