...
async function customizePhoneTypeSectionAndCreateTemplateNote(page: Page) {
  ...
  await page.getByTestId('workspace-type-schema-property-value-input').fill('https://example.com/phone-runbook')
  await sheet.getByRole('button', { name: 'Add property' }).click()
  await expect(page.getByTestId('workspace-type-schema-property-url')).toContainText('https://example.com/phone-runbook')
