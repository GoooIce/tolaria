...
import {
  ...
  nativeWysiwygFormatCommandLogPrefix,
  parseNativeWysiwygFormatCommandProofs,
} from '../src/qa/nativeWysiwygFormatCommandProbe.ts'
...
Usage:
  node apps/mobile/scripts/assert-ios-wysiwyg-format-commands.mjs [options]
...
async function openProbeUrl(device, openUrl, waitMs) {
  assertNativeQaOpenUrl(openUrl, 'Native iOS WYSIWYG format commands')
  terminateExpoGo(device)
  await sleep(500)
  run('xcrun', ['simctl', 'openurl', device, formatCommandProbeUrl(openUrl)])
  await sleep(Math.max(waitMs, minimumOpenWaitMs))
}

function formatCommandProbeUrl(openUrl) {
  return appendQueryParam(appendQueryParam(openUrl, 'wysiwygFormatCommandProbe', '1'), 'qaRun', Date.now().toString())
}
...
function collectSimulatorLogs(device, { last, start }) {
  return run('xcrun', [
    ...
    'compact',
    '--predicate',
    `eventMessage CONTAINS "${nativeWysiwygFormatCommandLogPrefix}"`,
  ])
}
...
async function main() {
  ...
  console.log('Native iOS WYSIWYG format command proof passed.')
}
